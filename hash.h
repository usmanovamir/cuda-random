...
//__device__ __forceinline__ void SHA256Initialize(uint32_t *s) {
//        s[0] = 0x6a09e667;
//        s[1] = 0xbb67ae85;
//        s[2] = 0x3c6ef372;
//        s[3] = 0xa54ff53a;
...
__device__ __forceinline__  void SHA256Transform(uint32_t* s, uint32_t* w) {
...
#define RIPEMD160Initialize(s) {\
(s)[0] = 0x67452301ul; \
(s)[1] = 0xEFCDAB89ul; \
(s)[2] = 0x98BADCFEul; \
(s)[3] = 0x10325476ul; \
(s)[4] = 0xC3D2E1F0ul; }
//__device__ __forceinline__  void RIPEMD160Initialize(uint32_t *s) {
//
//    s[0] = 0x67452301ul;
//    s[1] = 0xEFCDAB89ul;
//    s[2] = 0x98BADCFEul;
...
__device__ __forceinline__ void GetAddrHashComp(const uint64_t* x, uint8_t isOdd, uint8_t* hash, uint32_t* publicKeyBytes) {
...
#pragma unroll 8
    ...
    *(uint64_t*)(s + 10) = 0ULL;
    *(uint64_t*)(s + 12) = 0ULL;
    *(uint64_t*)(s + 14) = 256ULL;

    RIPEMD160Initialize((uint32_t*)hash);
    RIPEMD160Transform((uint32_t*)hash, s);
}

__device__  void GetAddrHashUncomp(const uint64_t* x, const uint64_t* y, uint8_t* hash, uint32_t* publicKeyBytes) {
